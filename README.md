...
대시보드 수집 버튼은 정상 동작하던 고급 IWR6843 파서를 실행합니다.
한 번의 수집으로 기존 UI용 flat CSV와 ML/공간 분석용 structured CSV를 동시에 만듭니다.

예를 들어 `runtime/collection_sensor_01_YYYYMMDD_HHMMSS.csv`가 UI용 flat CSV이고,
같은 prefix로 아래 파일들이 함께 생성됩니다.

```text
runtime/collection_sensor_01_YYYYMMDD_HHMMSS_frames.csv
runtime/collection_sensor_01_YYYYMMDD_HHMMSS_points.csv
runtime/collection_sensor_01_YYYYMMDD_HHMMSS_targets.csv
runtime/collection_sensor_01_YYYYMMDD_HHMMSS_target_indexes.csv
runtime/collection_sensor_01_YYYYMMDD_HHMMSS_vitals.csv
```

수집을 중지하면 서버가 자동으로 5초 단위 grid window feature도 생성합니다.

```text
runtime/collection_sensor_01_YYYYMMDD_HHMMSS_grid_windows.csv
```

수동으로 같은 파서를 직접 실행할 때:

```bash
python3 radar_to_esp32.py \
...
`scripts/radar_dataset_capture.py`는 같은 파서의 보조 복사본입니다. 일반 실행은 저장소 루트의
`radar_to_esp32.py`를 사용하세요.

수동으로 grid window feature를 다시 생성할 때:
